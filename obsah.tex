...
Hlavním zamìøením této práce je testování webových slu¾eb. Webové slu¾by dnes patøí mezi populární technologii v oblasti vzdálené komunikace a distribuovaných technologii. Pou¾ívají se ke zveøejnìní rùzných aplikací na webu nebo také k propojení systémù, které pou¾ívají rùzné technologie. Zmínìné pou¾ití pøedurèuje technologii webových slu¾eb k pou¾ití více u¾ivateli v urèitý èasový okam¾ik a náchylnosti na chyby bìhem komunikace. Kvùli jejich popularitì jsou kladeny vysoké nároky na spolehlivost a odolnost. Pro zaruèení tìchto po¾adavkù je nutné webovou slu¾bu podrobit komplexním testùm, které provìøí danou slu¾bu pøed nasazením.

Tato práce navazuje na výsledky diplomové práce na téma injekce poruch pro webové slu¾by. Výsledkem pøedchozí práce byl nástroj pro injekci poruch do komunikace webových slu¾eb, který je mo¾né pou¾ít pro testování odolnosti. Tato práce se bude dále zabývat testováním odolnosti webových slu¾eb. Zamìøí se v¹ak více na chování slu¾eb pøi vysoké zátì¾i. Dal¹ím cílem této práce je spojit mo¾nosti zátì¾ového testování s injekcí poruch a zkoumat odolnost testované webové slu¾by.

Tato práce se nejdøíve zabývá pou¾ívanými technologiemi pro implementaci webových slu¾eb. Jsou pøedstaveny základní architektury a pøístupy, které jsou v dne¹ní dobì pou¾ívány pro tvorbu webových slu¾eb. Jeliko¾ hlavním tématem je testování webových slu¾eb, práce se dále zabývá testováním software obecnì. Dále jsou pøedstaveny rùzné postupy pøi testování webových slu¾eb, aby bylo zøejmé, jak se postupuje pøi testování webových slu¾eb. V dal¹ích kapitolách je pak pøedstaven návrh a implementace nástroje pro testování zátì¾e webových slu¾eb, který vyu¾ívá injekci poruch. V závìru práce je uvedeno shrnutí výsledkù, které byly dosa¾eny pou¾itím vytvoøeného nástroje.



\chapter{Základní pojmy}
V této kapitole je popsána vìt¹ina dùle¾itých pojmù a zkratek, které se vyskytují dále v práci nebo jsou dùle¾ité pro objasnìní nìkterého z pou¾itých principù.  Dále jsou vysvìtleny principy webových slu¾eb a pou¾itelné pøístupy. V dal¹í èásti pak jsou popsány principy testování softwaru. V poslední èásti základních pojmù jsou popsány souèásti platformy programovacího jazyka Java.

\subsubsection*{Hypermédia}
Pojem hypermédia \cite{hypermedia} poprvé pou¾il Ted Nelson jako zobecnìní pojmu hypertext. Hypertext jsou textové dokumenty propojené pomocí odkazù, zatímco hypermédia roz¹iøuje pojem hypertext na jakoukoliv formu média. Tedy jsou to média propojené pomocí odkazù na jiná média.
...
\begin{figure}[h]
\begin{center}
 \includegraphics[scale=1, bb= 0.0mm 235.0mm 130mm 294mmm]{pic/SOAPZprava.pdf}
...
\begin{figure}[h]
\begin{center}
 \includegraphics[scale=0.7, bb= 0.0mm 220.0mm 210mm 294mmm]{pic/SOAPhandler.pdf}
...
\item {\bf Bezstavovost}\\
Tato vlastnost u¾ musí být implementována specificky na dané architektuøe, která je postavena nad HTTP protokolem. Pokud je kontext ukládán na serveru, tak se nejedná o aplikaci zalo¾enou striktnì na principech REST.  
\item {\bf Mezi-pamì»}\\
...
Data, která jsou odesílána pomocí HTTP mohou být pøená¹ená v jiné reprezentaci ne¾ jak jsou ulo¾ena. Data je mo¾no pøená¹et ve standardizovaném formátu (JSON, XML apod.).
\item {\bf Sebe popisující zprávy }\\
Tuto vlastnost HTTP protokol implicitnì nesplòuje, ale umo¾òuje. Tuto vlastnost je tøeba doplnit ve vrstvì, která je implementována nad HTTP protokolem.
...
\noindent Nyní jsme si ukázali, ¾e HTTP protokol je mo¾no pou¾ít pro aplikace zalo¾ených na REST principech. 
 ...
 Základní princip webových slu¾eb je zprostøedkování aplikaènì nezávislého komunikaèního rozhraní. Pro vytvoøení jednotného a aplikaènì nezávislého komunikaèního rozhraní je mo¾né pou¾ít HTTP protokol, a zprostøedkovat tak aplikace, které jsou vytvoøeny rùznými technologiemi. HTTP disponuje sadou pøíkazù, které je mo¾no vyu¾ít právì ke konstrukci webových slu¾eb respektující omezení REST. Základní pøíkazy, které se nejèastìji pou¾ívají jsou GET, PUT, POST, DELETE. Tato skupina operací se oznaèuje akronymem CRUD (Create, Read, Update, Delete ) a je mo¾né se s ní setkat v deklarativních zpùsobech programování.
...
V ideálním pøípadì bychom pøi testování vyzkou¹eli v¹echny mo¾né zpùsoby pou¾ití, vstupy, prostøedí, abychom dùkladnì otestovali vytvoøený program. V reálném svìtì tento zpùsob není mo¾ný, proto¾e vyzkou¹et v¹echny mo¾né vstupy i jednoduchého programu je prakticky neøe¹itelný problém. K tomuto tvrzení staèí poukázat na program, který seète dvì èísla z mno¾iny reálných èísel. Ne v¾dy je kompletní testování neøe¹itelný problém, minimálnì je v¹ak kompletní testování velmi nepraktické a hlavnì neekonomické.
...
\subsection*{Internet}
U veøejnì dostupných webových slu¾eb je obtí¾nìj¹í odhadnout kolik u¾ivatelù bude tuto slu¾bu vyu¾ívat. V tomto pøípadì nemusejí být tak vysoké nároky na zabezpeèení jako na webové slu¾by dostupné z intranetu.

\section{Izolace webových slu¾eb}
Pøi testování webových slu¾eb je v nìkterých pøípadech potøeba cílovou webovou slu¾bu izolovat. Izolace webové slu¾by se vyu¾ívá v pøípadech, kdy je potøeba simulovat propojení s dal¹ími slu¾bami nebo systémy. Tyto slu¾by jsou simulovány tzv. {\it Mock} slu¾bami. {\it Mock}  slu¾ba disponuje shodným rozhraním jako její vzorová slu¾ba, av¹ak její funkcionalita není implementována a je omezena na konstantní odpovìï. Toto øe¹ení je výhodné v testovacích prostøedích z více dùvodù. Vzorové slu¾by mohou být placené, jejich implementace nemusí být veøejná, mají del¹í èas odezvy ne¾ Mock slu¾by, Mock slu¾by dovolují testovat jednotlivé celky systému oddìlenì apod.
...
V této èásti bude roz¹íøeno a upøesnìno oficiální zadání.  Specifikace po¾adavkù je dùle¾itou souèástí vývoje softwaru, proto¾e v této fázi jsou jednoznaènì definovány po¾adavky na vytváøený produkt. Nejdøíve budou specifikovány základní funkce nástroje. Dal¹í dvì èásti detailnìji specifikují po¾adavky ze zadání. Dále pak bude následovat roz¹íøení v podobì distribuované architektury. 


\section{Základní koncept}
Základní koncept obsahuje základní po¾adavky, bez kterých by nástroj byl nefunkèní. Bude obsahovat po¾adavky na klientskou aplikaci. 
...
Nástroj bude mo¾no ovládat pomocí pøíkazové øádky. Z pøíkazové øádky bude mo¾né ovládat základní funkce nástroje tzn. konfigurace testù, spu¹tìní testù, ulo¾ení výsledkù testù. 
...
U¾ivatelské rozhraní bude vytvoøeno v grafické podobì i v podobì ovládání z pøíkazové øádky. Testovací jednotka se bude moci ovládat pomocí dvou rozhraní.
...
Tento koncept lze také pou¾ít pro vytvoøení distribuované architektury, proto¾e dovolí oddìlit testovací jednotku od u¾ivatelských rozhraní. Oddìlení bude spoèívat v lehce rozdílném komunikaèním rozhraní. Testovací jednotka spoleènì s proxy jednotkou dostupné pøes vzdálenì dostupný proces, který bude mít Java RMI rozhraní a bude muset být spu¹tìn døíve ne¾ se s ním bude  vzdálenì manipulovat.
...
Testovací data se budou vytváøet v projektech. Ne¾ u¾ivatel bude moci zahájit jakoukoliv práci, musí vytvoøit nový projekt nebo pou¾ít ji¾ døíve vytvoøený. Struktura projektu bude obsahovat sada testù a jednotlivé testy, které mohou být vytvoøeny uvnitø sady testù. Struktura a nastavení projektu bude popsána v XML dokumentu. 
...
Centralizovaná správa v¹ech jednotek je vhodná kvùli vytváøením a následné distribuci testovacích dat. V centrálním bodì se vytvoøí testovací data a roze¹lou se na vzdálené uzly, kde budou moci být vyu¾ity. Výsledky testù budou zpìt odeslány okam¾itì po jejich získání a ulo¾eny pro zobrazení a vyhodnocení u¾ivatelem.

\section{Pou¾ití injekce chyb}	
HTTP proxy jednotka bude vytvoøena ze zdrojových kódù pøedchozí práce a bude pøizpùsobena ke samostatnému spu¹tìní. Funkce proxy jednotky bude spoèívat v zachycení komunikace mezi testovací jednotkou a webovou slu¾bou. Tento princip lze vyu¾ít k zátì¾ovému testováni spojeném s injekcí chyb. 

\subsection*{Distribuovaný re¾im}
...
\subsection*{Vylep¹ení jádra proxy jednotky}
...
Nyní se zamìøíme na zpracování HTTP zprávy. Polo¾ka {\it Content-Encoding} udává zakódování odesílaného obsahu na stranì koncových bodù. Zpùsob komprimace uvedený v této polo¾ce probíhá {\bf pouze} na stranì klienta nebo serveru. 

Polo¾ka {\it Transfer-Encoding} udává zakódování obsahu zprávy bìhem transportu sítí. Komprimace mù¾e probìhnout kdekoliv na cestì zprávy mezi  koncovými body, vèetnì koncových bodù samotných. Tento zpùsob zakódování probíhá a¾ po komprimaci udávané v polo¾ce {\it Content-Encoding}. Pokud jsou v hlavièce HTTP zprávy obì polo¾ky znamená to, ¾e zpráva byla zakódována nejdøíve zpùsobem uvedeným v polo¾ce {\it Content-Encoding} a následnì je¹tì zpùsobem uvedeným v polo¾ce {\it Transfer-Encoding}. Zpráva mù¾e mezi koncovými body být zpracována rùznými procesy, kdy poka¾dé mù¾e dojit ke zmìnì zakódování. Bìhem pøenosu se v¹ak mìní pouze pozdìj¹í zpùsob zakódovaní, tedy {\it Transfer-Encoding}.
\\\\
\noindent Mo¾nosti pro vylep¹ení zpracování HTTP zprávy proxy jednotky:
\begin{itemize} 
\item{ \bf Komprimace zpráv}\\
Obsah HTTP zprávy je mo¾no pøed odesláním komprimovat. Èasto pou¾ívanými zpùsoby jsou {\it GZip} a {\it Deflate}. Pokud se do hlavièky uvede zpùsob komprimace a daný interpret je schopný tento zpùsob zpracovat, je mo¾né pou¾ít libovolný zpùsob komprimace.  Proxy jednotka bude vylep¹ena o zpracování zpùsobù {\it GZip} a {\it Deflate}. Bude schopná zprávu dekomprimovat, analyzovat, naru¹it, následnì komprimovat a odeslat. Pro komprimaci a dekomprimaci  lze pou¾ít volnì dostupné knihovny.   \\
\item{ \bf Odesílání po èástech}\\
Mo¾nost odesílat zprávu po èástech ({\it chunked encoding}) byla zavedena do HTTP protokolu od verze 1.1 \cite{HTTP}. To je výhodné v pøípadech nároènìj¹ího zpracování dat, kdy se nemusí èekat a¾ jsou v¹echna data zpracována do jedné zprávy. Pøi odesílání po èástech je odeslána HTTP hlavièka a za ní jsou odeslány jednotlivé èásti. Jednotlivé èásti zprávy jsou oddìlené prázdným øádkem. Ka¾dá èást zprávy má na prvním øádku uvedenou svou velikost a na dal¹ím øádku je obsah pøíslu¹né èásti. Tento zpùsob pøenosu v¹ak výraznì zvy¹uje obtí¾nost injekce chyby. Proto proxy jednotka poskládá jednotlivé jednotlivé èásti dohromady, provede analýzu a pøípadné naru¹ení a ode¹le zprávu dále jako celek. Je také vhodné obsah zprávy pøed odesláním komprimovat.
\item{ \bf Monitorování bez injekce}\\
V pøede¹lé verzi proxy jednotky nebylo mo¾né zachycovat zprávy bez mo¾nosti naru¹ení. Byly tedy zaznamenány pouze zprávy, které vyhovìly podmínce v injektoru chyb. Toto chování bude vylep¹eno a bude zaznamenán ve¹kerý provoz. Pokud probìhne injekce chyby, bude nastaven pøíznak naru¹ení zprávy. \\
\end{itemize}
...
Pøede¹lá verze podporovala zpracování pouze formátu XML. V ostatních pøípadech nebyla data proxy jednotkou vùbec zaznamenána. Nyní proxy jednotka bude nejprve èíst obsah parametru {\it Content-Type} a podle toho rozhodne jakým zpùsobem data zpracuje. Pokud parametr bude v daném parametru neznámá hodnota nebo nebude pøítomen vùbec, bude se zprávou zacházeno jako s neformátovaným textem.\\\\
\noindent Dal¹í mo¾nosti pro podporu dal¹ích datových formátù injektoru poruch:
\begin{itemize} 
\item{ \bf JSON}\\
JSON je roz¹íøený datový formát, který èasto pou¾ívají webové slu¾by REST charakteru. Data reprezentována JSON formátem jsou v textové podobì. Lze tedy uplatnit stávájící injekce poruch. Pokud v¹ak je nutné komplexnìj¹í injekci, jako napøíklad zmìna urèité promìnné, je nutné nejdøíve zpracovat data vhodným zpùsobem. Pro komplexnìj¹í zpracování je mo¾no vyu¾ít volnì dostupných knihoven.\\
\item{ \bf YAML}\\
YAML \cite{yaml} je svou charakteristikou i pou¾itím velmi podobný formátu JSON. Také se jedná o reprezentaci dat v textovém formátu. Bohu¾el nebyla nalezena knihovna, která by umo¾òovala zpracování dle modelu YAML bez pøetypování na známý Java objekt nebo kolekci. Pokud není známa implementace objektu, není mo¾né jednoznaènì urèit typ objektu, na který má být pøeètená data pøetypována. Dostupné knihovny pro JSON formát umo¾òují manipulaci s JSON pomocí modelu, tedy není nutné pøetypování na známý objekt nebo kolekci. Tento formát tedy nebude proxy jednotkou pøímo podporován. Z vý¹e uvedených dùvodù bude s tímto formátem zacházeno jako s neformátovaným textem.  \\
\end{itemize}
...
\subsection*{Pøidání podmínek a poruch}

\begin{itemize} 
\item{ \bf Podmínka: Test existence polo¾ky v hlavièce HTTP zprávy}\\
Bude pøidána podmínka pro detekci polo¾ky v hlavièce HTTP zprávy. Bude mo¾no zadat název parametru a jeho hodnotu. Pokud nebude nebude hodnota zadána bude se testovat pouze výskyt parametru.
\item{ \bf Porucha: Vlo¾ení hodnoty do promìnné v JSON formátu}\\
Jeliko¾ bude pøidána podpora pro datový formát JSON, bude mo¾no zadat název promìnné, kam se vlo¾í po¾adovaná hodnota. Tato porucha byla zvolena, proto¾e neoèekávané hodnoty, doká¾í vyvolat neoèekávané chování pøi nedostateèném o¹etøení vstupù. Obzvlá¹tì pak hodnota {\it null} je velmi kvalitním pøíkladem.
\end{itemize}
...
Tato vrstva je zodpovìdná za distribuci dat v rámci aplikace. Bude pøijímat data od sousedních vrstev a plnit funkci prostøedníka, pøi pøedávání dat mezi prezentaèní  a datovou vrstvou. V této vrstvì se nachází také aplikaèní jednotky, které jsou zodpovìdné za komunikaci s webovými slu¾bami.    \\
...
V této sekci budou spojeny jednotlivé postupy z pøede¹lých sekcí a bude pøedstaven detailní návrh architektury vytváøeného nástroje. V sekci jsou nastínìny jednotlivé souèásti nástroje a jak spolu mají komunikovat. Aby tato komunikace ¹la snadno implementovat byl pøedstaven návrh vrstev, do kterých budou jednotlivé souèásti rozmístìny, tak aby spolu komunikovaly pouze sousední vrstvy. Tento zpùsob návrhu je výhodný kvùli rozdìlení slo¾itìj¹ích problému na podproblémy. Dekompozice také usnadòuje pøípadné testování jednotlivých souèástí aplikace (Unit testing).

Jeliko¾ tato práce navazuje na ji¾ vytvoøenou implementaci, která nevyhovovala striktnímu rozdìlení do vrstev, bylo nutné vytvoøit návrh úprav, který je na obrázku \ref{fig:zmena} . Po provedení úprav bude mo¾no vyu¾ít stávající implementaci a pøidat k ní dal¹í prvky, které vy¾aduje zadání práce.
...
V dne¹ní dobì vìt¹ina programovacích jazykù má implementovánu podporu pro vývoj aplikací pou¾ívající webové slu¾by. Mezi nimi v¹ak vyènívají platformy .NET a Java, které mají nej¹ir¹í podporu pro vývoj aplikací pro webové slu¾by.  Vzhledem k návaznosti této práce na práci, která je implementována na platformì Java, je vhodnìj¹í pou¾ití platformy Java. Platforma Java byla také zvolena kvùli zku¹enostem autora.

Pøi implementaci jsou pou¾ity souèásti Java SE a pro vytvoøení testovacích pøíkladù je pou¾ito souèástí Java EE. Jako prostøedí pro vývoj aplikace je mo¾no pou¾ít libovolný nástroj. Pro urychlení vývoje je v¹ak vhodné pou¾ít integrované vývojové prostøedí (IDE). Velice známá jsou prostøedí NetBeans a Eclipse. Obì jsou multiplatformní a poskytují nástroje pro vývoj aplikací na platformì Java. Bylo zvoleno integrované prostøedí Eclipse. Eclipse prostøedí bylo zvoleno ze subjektivních dùvodù, proto¾e autor má toto prostøedí v oblibì.

\newpage
\section{Základní model aplikace}
V návrhu na obrázku \ref{fig:zaklad} byl pøedstaven základní model aplikace. Dále pak na obrázku \ref{fig:modelImpl} bylo popsáno jak by tento koncept mìl být rozdìlen do vrstev. V této èásti je znázornìno jak jsou tyto dva pohledy na návrh implementovány. Jednotlivé èásti budou podrobnì popsány a dále v textu budou uvedeny dal¹í diagramy vysvìtlující tok dat a pøíkazù mezi jednotlivými souèástmi.

\subsection*{Reprezentace dat}
Pro organizaci dat bylo zavedeno hierarchické schéma, které je patrné na obrázku \ref{fig:schema}. Na nejvy¹¹í úrovni je koøenový adresáø, který není v grafickém u¾ivatelském rozhraní (dále jen GUI) patrný. Koøenový adresáø se nachází v pracovním adresáøi aplikace a pokud není pøi startu aplikace pøítomen, je adresáø vytvoøen.
...
Jednotlivé testovací pøípady pøedstavují dal¹í úroveò schématu. Ka¾dý testovací pøípad obsahuje slo¾ky pro vstupní a výstupní data pro aplikaèní jednotky. Tyto slo¾ky jsou zobrazeny v GUI jako {\it FaultInjection} a {\it Http}. Testovací pøípad dále obsahuje XML soubor {\it settings.xml}, kde jsou uvedeny nezbytná nastavení pro testovací pøípad. Obsah souboru s nastavením a slo¾ek pro aplikaèní jednotky je popsán ní¾e.
\begin{itemize}
\item {\bf Data pro nastavení testu }\\
Data pro nastavení testu obsahují informace, podle kterých se nastaví aplikaèní jednotky. V souboru {\it settings.xml}  je ulo¾ena instance tøídy {\it TestCaseSettingsData}, která nese informace pro spu¹tìní pro testovací a proxy jednotku. Nastavit je mo¾né spu¹tìní testovací jednotky, èasový limit pro pøijetí odpovìdi, poèet opakovaní testu, poèet vláken spu¹tìných testem. Dále je mo¾né nastavit spu¹tìní proxy jednotky, pro kterou je nutné uvést adresu, port a port testované slu¾by.  \\
\item {\bf Data pro testovací jednotku }\\
Data pro  testovací jednotku rozdìlena do slo¾ek pro vstupní ({\it input}) a výstupní data ({\it output}). Tyto slo¾ky nejsou v GUI zobrazeny. Ve slo¾ce pro vstupní data je XML soubor {\it input.xml}, kde jsou umístìna data pro testovací po¾adavek.  Do slo¾ky pro výstupní data se pak ukládají výsledky testù. Data ulo¾ená v souboru {\it input.xml} pøedstavuje serializovanou instanci tøídy {\it HttpMessageData}, která nese potøebné informace o HTTP po¾adavku. Náhled zadání dat pro HTTP po¾adavek je na obrázku \ref{fig:http} \\
\item {\bf Data pro injekci chyb }\\
Data pro injekci chyb jsou rozdìlena stejným zpùsobem jako data pro testovací jednotku. Data ulo¾ená v souboru  {\it input.xml} je pøedstavuje serializovanou instanci tøídy {\it FaultInjectionData}, která nese seznam podmínek a naru¹ení pro injektor poruch.
\\
\end{itemize}

\subsection*{Zprostøedkovatel dat}
Zprostøedkovatel dat je reprezentován tøídou {\it DataProvider}, která byla ji¾ v pøedchozí implementaci. Tato tøída má na starosti  serializaci a deserializaci Java objektù do respektive z XML formátu. K tomuto druhu manipulace je pou¾ita knihovna Xstream. Ve¹kerá data jsou ulo¾ena na disku ve formì textových dat ve formátu XML. Pokud chce nástroj data pou¾ívat, data naète jako textový øetìzec a pomocí knihovny Xstream dojde k deserializaci. Pøesnì opaènì se postupuje pøi zápisu instancí tøíd na disk.

 V pøedchozí implementaci byly metody z této tøídy volány pouze staticky. Pro úèely nové implementace byla tato tøída pøepracována a metody jsou volány z instance dané tøídy.

\begin{figure}[h]
\begin{center}
 \includegraphics[scale=0.3]{pic/menu2.jpg}
...
\subsection*{Grafické u¾ivatelské rozhraní}
Grafické u¾ivatelské rozhraní je vytvoøeno v sadì nástrojù Swing. Na obrázku \ref{fig:testunit} je vidìt základní rozlo¾ení GUI. Na levé stranì je navigaèní panel, který zobrazuje obsah koøenového adresáøe. Ve spodní èásti je mo¾no vidìt konzolu, kam se zapisují vygenerované zprávy nástroje. Zbytek plochy je hlavní panel.
...
Navigaèní panel je tvoøen tøídou {\it Navigator}, která umo¾òuje vyvolání kontextového menu pro intuitivnìj¹í pou¾ití. Pomocí kontextového menu je mo¾no otevírat jednotlivé hlavní panely podle toho, na kterou polo¾ku ve struktuøe bylo kliknuto. Pøi vytváøení nových polo¾ek je nutno nejdøíve vybrat polo¾ku nadøazeného prvku. Jedinou výjimkou je vytvoøení testovací sady, proto¾e je hierarchicky nejvý¹e a nemá nadøazený prvek. Pokud se u¾ivatel pokusí vytvoøit polo¾ku bez vybrání nadøazeného prvku, bude do konzole vypsána patøièná hlá¹ka. V pøípadì testovacího pøípadu jej nebude mo¾né vytvoøit bez oznaèení testovací sady, do které má být vytvoøen.
...
Testovací panel je hlavní zobrazovací panel aplikace, odkud se ovládá spou¹tìní testovacích po¾adavkù a jejich vyhodnocení, viz obrázek \ref{fig:testunit}. Panel obsahuje li¹tu s pøíkazy pro ovládání aplikace, tabulku pro testovací pøípady, tabulku pro výsledky testù a panely pro zobrazení obsahu odpovìdí. 
...
Na panelu nástrojù je vytvoøena je¹tì funkce pro export konfigurace distribuované ho testu. Tento export ulo¾í údaje o spojení ke vzdáleným strojùm do souboru. Pak je mo¾né pomocí importu konfigurace pøipojit vzdálené stroje pomocí jednoho pøíkazu. Hlavním dùvodem exportu konfigurace je v¹ak mo¾nost spu¹tìní distribuovaného testu z pøíkazové øádky. Pro pøipojení vzdálených strojù pøi spu¹tìní testu z pøíkazové øádky bude pou¾ito právì exportované konfigurace. Pro ulo¾ení tìchto dat byla pou¾ita knihovna {\it ini4j}. Tato knihovna slou¾í k ukládání dat ve formátu {\it ini}. Tento formát byl zvolen kvùli jednoduchosti jeho pøípadné editace u¾ivatelem nebo jeho vytvoøení bez pou¾ití GUI.


\subsubsection*{Panel proxy monitoru}

V zobrazovacím panelu proxy monitorovací jednotky bylo provedeno jen nìkolik zmìn, viz obrázek \ref{fig:testunit}. Zmìny v zobrazovacím panelu se týkají ovládání jednotky. Zadání adresy a portu, bylo pøesunuto do dat pro testování. Pou¾itý testovací pøípad se novì vybírá z navigaèního panelu. V pøede¹lé implementaci se vybíraly z vysouvací nabídky. Proxy monitorovací jednotku lze spustit i samostatnì bez pøítomnosti testovací jednotky. Je nutno oznaèit test v navigaèním panelu a spustit jednotku. Samostatnì lze spou¹tìt i proxy jednotky na vzdálených strojích. Uplatní se stejný postup, který je popsán v pøedchozím odstavci pro testovací jednotku.
...
Dále pak v pùvodní implementaci mìla proxy jednotka za úkol monitoring pøíchozích spojení a vytvoøení vláken pro obsluhu spojení. Ve¹kerá delegaèní logika pro odeslání dat do zobrazovacích panelù byla implementována v kontroléru. Tato funkcionalita byla pøesunuta do proxy jednotky. Mù¾e se zdát, ¾e tato funkcionalita by nemìla být souèástí proxy jednotky, ale díky tomuto øe¹ení je mo¾né oddìlit proxy jednotku od kontroléru. Nyní zachycená data odesílá proxy jednotka pøes delegaèní instanci pøímo do zobrazovacího panelu, viz obrázek \ref{fig:distmodel}. Takto upravená implementace jednotky mù¾e být spu¹tìná na vzdáleném stroji a zaregistrovat zobrazovací panel pomocí RMI metod.



\section{Vylep¹ení proxy jednotky}
V této èásti bude popsána implementace navr¾ených zmìn v proxy jednotce. Nejdøíve bylo nutné upravit zpracování HTTP zprávy. Po vylep¹eném pøijmu HTTP zprávy je mo¾né detekovat datový formát dané zprávy a podle toho pøizpùsobit zpracování dané zprávy. Jakmile mù¾eme zpracovat rùzné datové formáty mù¾eme adekvátnì pøizpùsobit i analýzu a naru¹ení pøijaté zprávy. 

\subsection*{Zpracování HTTP zpráv}
Pro implementaci komprimaèních metod {\it GZip} a {\it Deflate} jsou pou¾ity knihovny ze standardní sady jazyka Java (JDK, Java Development Kit). 

Pro implementaci zpracování zprávy po èástech je nutno upravit algoritmus ètení zprávy. Nejprve je tøeba detekovat, ¾e se jedná o zpracování èástech. Pak se ètou jednotlivé èásti zpùsobem, který je popsán v návrhu. Po pøeètení celé zprávy je se zprávou nalo¾eno jako s bì¾nì pøeètenou zprávou.
\subsection*{Podpora dal¹ích datových reprezentací }
Pro zpracování obsahu ve formátu JSON je pou¾ita knihovna  {\it GSON} \cite{gson}. Podpora formátu v rámci vytvoøeného nástroje znamená, ¾e textové výstupy tohoto formátu jsou pøehlednì naformátovány a je mo¾né pou¾ít obsah zpráv ve formátu JSON pøi injekci poruch.

\subsection*{Pøidání dal¹ích typù injekcí chyb a jejich podmínek}
Pro implementaci dal¹ích funkcí injektoru poruch bylo nejdøíve potøeba vytvoøit mo¾nost zadání dat pro tyto funkce. Do modálního okna pro zadání podmínky injekce  byla pøidána podmínka {\it HeaderCondition}. Pro implementaci funkce této podmínky je implementováno rozhraní  {\it Condition}. Nová tøída je nazvána {\it HeaderCondition}.

Pro implementaci naru¹ení zprávy ve formátu JSON byla také pøidána polo¾ka do výbìru poruch pøi zadávání naru¹ení injektorem. Pro implementaci dal¹í poruchy je implementováno rozhraní {\it Fault}. Nová tøída je nazvaná {\it JSONFault}. Aplikace chyby spoèívá v pøevedení textové reprezentace na objekt, který reprezentuje model formátu JSON. Díky manipulaci s modelem modelem formátu JSON je mo¾né najít potøebnou promìnnou a zmìnit její hodnotu.
...
\caption{Model delegace zobrazení dat pøijatých aplikaèními jednotkami. }
\label{fig:distmodel}
\end{center}
\end{figure}

\section{Implementace distribuované architektury}
Tato sekce je zamìøena na detaily implementace distribuované architektury vytvoøeného nástroje. Pro implementaci spojení mezi jednotlivými vzdálenými stroji byla vybrána technologie Java RMI \cite{rmi}. Nynìj¹í implementace Java RMI splòuje standard CORBA. Pøi dodr¾ení rozhraní lze komunikovat se vzdálenou stranou, která je implementována jinou technologii (C++, Python apod.). Java RMI je technologie umo¾òující volání metod objektù, které jsou umístìny na vzdáleném stroji. Staèí tedy spustit RMI server a registrovat objekt. Po¾adovaný objekt je zaregistrován v registru. Registr je mo¾né kontaktovat a vy¾ádat si rozhraní daného objektu.


\noindent Na obrázku \ref{fig:distmodel}. je zobrazen model implementace distribuované architektury vytvoøeného nástroje. Vzdálené stroje jsou implementovány jako RMI server. Ka¾dý server musí mít vlastní registr. Centrální GUI tedy plni roli klienta. V tomto pøípadì jsou instance vzdálených aplikaèních jednotek uchovány v ADT {\it HashMap}. Jednotlivé instance jsou spárovány se zobrazovacím panelem identifikaèním èíslem. Pøi interpretaci pøíkazu z GUI je získáno identifikaèní èíslo z hlavièky panelu. Identifikaèní èíslo je pak pou¾ito jako klíè k vyhledání správné aplikaèní jednotky. Po získání instance aplikaèní jednotky je pøíkaz odeslán.



\chapter{Prùbìh testování webových slu¾eb}
Tato kapitola se zabývá pou¾itím vytvoøeného nástroje. V pøedchozí práci \cite{predchozi} bylo pøedlo¾eno schéma {\it wsCrash} pro klasifikaci poruch, které vychází z této práce \cite{4272964}. Tato práce se této klasifikace bude dr¾et a pøi testování se bude sna¾it vzniklé chyby zaøadit do této klasifikace. 
\begin{itemize}
\item {\bf Catastrophic} - Aplikaèní server, na kterém bì¾ela slu¾ba, musel být restartován nebo byl po¹kozen.\\
\item  {\bf Restart} - Slu¾ba je pozastavena nebo musí být ukonèena.\\
\item  {\bf Abort} - Neoèekávané chování webové slu¾by. Napøíklad ukonèení spojení bez chybové zprávy.\\
\item  {\bf Silent} - V tomto pøípadì by mìla být vráceno chybové hlá¹ení, ale slu¾ba vrací standardní odpovìï. \\
\item  {\bf Hindering} - Slu¾bou je vrácen nesprávný chybový kód nebo odpovìï slu¾by je významnì zpo¾dìna.
\end{itemize}
Zátì¾ové testování bude nejprve zkoumat, jestli slu¾ba správnì zpracovává více po¾adavkù v krátkém èasovém okam¾iku. Tímto zpùsobem mù¾eme ovìøit základní pøedpoklad správného zpracování více po¾adavkù. Následnì mù¾eme spustit zátì¾ový test a hledat hranici pou¾itelnosti na základì doby odezvy testované slu¾by. Hranici mù¾eme nastavit èasovým limitem HTTP spojení, po¾adavku a sledovat od jakého poètu odeslaných po¾adavkù a spu¹tìných vláken zaène klientská aplikace uzavírat spojení.
\newpage

\subsection*{Nastavení a spu¹tìní nástroje}
Pro zátì¾ový test je mo¾né vyu¾ít distribuované architektury a zapojit do testu více fyzických strojù. Nejdøíve je nutné spustit RMI server nástroje na vzdálených strojích, kde se instance aplikaèních jednotek zpøístupní pomocí RMI registru. Klientskou aplikaci i RMI server je {\bf nutné	} spou¹tìt s parametrem {\tt -Djava.rmi.server.hostname}, ve kterém je uvedena adresa rozhraní. 

\subsection*{Testovací prostøedí}
Pro ovìøení funkce nástroje byla vytvoøena ukázková webová slu¾ba. Jako aplikaèní server byl pou¾it JBoss AS verze 7.0.2, který je spu¹tìn na jiném fyzickém stroji ne¾ testovací nástroj. Slu¾ba {\it BookStore} pou¾ívá SOAP protokol, který pou¾ívá zprávy v XML formátu. Pro klientskou aplikaci byly k dispozici dva fyzické stroje jejich¾ konfigurace je uvedena ní¾e.


\section{Testování webových slu¾eb s  dostupným zdrojovým kódem}
Tato sekce se zabývá testováním slu¾eb se známým kódem. Vytvoøená slu¾ba jsou pouze ukázková aplikace, která slou¾í k demonstraci práce nástroje.
\subsection*{Zátì¾ový test SOAP webové slu¾by {\it BookStore}}
limit spojeni: 1000 ms, limit po¾adavku: 1000ms\\
\begin{tabular}{| c | c | c | c |}
\hline
poèet vláken & poèet opakování & limit spojení& limit po¾adavku\\ \hline
1&200 & 0&0\\ \hline
...
\hline
\end{tabular}\\\\
\noindent Z tabulky je patrné, ¾e slu¾ba zaèala po urèitém poètu po¾adavkù zvy¹ovat dobu odezvy nad èasový limit a spojení zaèaly pøeru¹ovat.

\section{Testování veøejnì dostupných slu¾eb}
Tato sekce se bude zabývat testováním webových slu¾eb, které jsou veøejnì dostupné a jejich implementace je neznámá. Pøi testování budou nastaveny èasové limity pro spojení a po¾adavek. Testování bude probíhat v iteracích, pøièem¾ se zátì¾ bude pøi ka¾dé dal¹í iteraci zvy¹ovat. Jakmile slu¾ba projeví známky pøetí¾ení, testování bude ukonèeno.

Slu¾by pro testování byly vybrány náhodnì a jejich názvy a adresy jsou uvedeny ní¾e. Bìhem prvních zku¹ebních testù bylo zji¹tìno, ¾e nìkteré slu¾by doká¾í vyhodnotit automatizovanì odesílané po¾adavky ji¾ pøi malém poètu (5 - 25). Pro detailnìj¹í zátì¾ové testování byly autorem vyhledány slu¾by, které touto kontrolou nedisponují.

\newpage
\subsection*{Zátì¾ový test REST webové slu¾by}

\noindent Název: ZipCode\\
\noindent URL: http://www.ripedevelopment.com/webservices/ZipCode.asmx
\\\\
\noindent limit spojeni: 3000 ms, limit po¾adavku: 1000ms\\
\begin{tabular}{| c | c | c | c |}
\hline
poèet vláken & poèet opakování & limit spojení& limit po¾adavku\\ \hline
1&200 & 0&0\\ \hline
...
\hline
\end{tabular}\\\\
Tato slu¾ba se jevila po prvních testech jako pomìrnì odolná, proto byl pøidán dal¹í fyzický stroj. Poslední test je tedy výsledkem zatí¾ení pomocí dvou fyzických strojù. Jeden stroj pracoval s 10 vlákny a 1000 po¾adavky na vlákno.


\subsection*{Zátì¾ový test SOAP webové slu¾by}

\noindent Název: INVOICE\\
\noindent URL: http://www.thomas-bayer.com/sqlrest/INVOICE/
\\\\
\noindent limit spojeni: 3000 ms, limit po¾adavku: 1000ms\\
\begin{tabular}{| c | c | c | c |}
\hline
...
Prùbìh testování ukázal, ¾e pøi nevhodné implementaci je mo¾né se setkat s chybným chováním slu¾by ji¾ pøi nízkém poètu vláken. Dále bylo zji¹tìno, ¾e pøi vy¹¹ím zatí¾ení je mo¾no pozorovat nìkolik negativních rysù chování slu¾by. V nejlep¹ím pøípadì je vrácen chybový kód 500, patøièná zpráva a pøi sní¾ení zatí¾ení slu¾ba funguje správnì. Dále bylo zji¹tìno, ¾e pøi vysokém zatí¾ení je mo¾né se setkat s dvìma druhy èasových limitù. Musíme pøedpokládat, ¾e jsou nastaveny na klientské stranì. V opaèném pøípadì mù¾e celá aplikace zamrznout na neurèitou dobu. Pokud je slu¾ba opravdu pøetí¾ena není s ní mo¾no navázat spojení a je uplatnìn èasový limit pro navázání spojení. Pokud zatí¾ení není tak kritické, je spojení navázáno, ale je mo¾né se setkat s dobou èekání na data v øádech sekund. Zde se uplatní èasový limit na HTTP po¾adavek. Ukonèení spojení díky uplatnìní èasového limitu je mo¾né zaøadit do kategorie {\it Abort}.
 
Dvì veøejnì dostupné slu¾by byly podrobeny detailnímu zátì¾ovému testování, proto¾e bìhem zku¹ebního testu neprojevily známky ochranných mechanismù. Tøetí slu¾ba {\it Search} od spoleènosti Google v¹ak ji¾ po nìkolika (5 - 25) po¾adavcích zaèala odesílat odpovìdi, kde byla uvedena zpráva o mo¾ném výskytu zneu¾ití slu¾by. Pøi této odpovìdi byl kód HTTP odpovìdi 200 (OK), ale v tìle odpovìdi byla polo¾ka {\it Status code} s èíslem 403 (Forbidden - zakázáno).


\chapter{Závìr}
Cílem práce bylo roz¹íøení výsledkù pøedchozí práce, která se vìnovala injekci poruch do komunikace webových slu¾eb. Roz¹íøení se mìlo zamìøit na sledování chování webových slu¾eb pøi vysoké zátì¾i a pokusit se odhalit mo¾né nedostatky v jejich chování. Nejdøíve se práce vìnovala úvodu do problematiky webových slu¾eb a testování software. Byly pøedstaveny základní architektury pro návrh webových slu¾eb a jednotlivé technologie, které se pou¾ívají pro jejich implementaci. Dále v této kapitole byly pøedstaveny základní principy pro testování jakéhokoliv software a jako poslední byla popsána platforma Java. Popis platformy Java byl zvolen, proto¾e je pou¾ita pro výslednou implementaci nástroje. Druhá èást práce se vìnuje problematice testování webových slu¾eb. Jsou popsány rùzné pøístupy k testování a jak testování ovlivòuje umístìní webové slu¾by. Na konci této kapitoly byly pøedstaveny ji¾ existující nástroje, které se pou¾ívají pro testování webových slu¾eb.

Dal¹í kapitola je vìnována specifikaci po¾adavkù, kde byly stanoveny po¾adavky na vytváøeny nástroj. Po specifikaci po¾adavkù se práce soustøedila na návrh jednotlivých souèástí nástroje a jakým zpùsobem budou pou¾ity výsledky pøedchozí práce. Byl pøedstaven návrh klientské aplikace, která má za úkol odesílat HTTP po¾adavky k testované webové slu¾bì. Návrh splòuje rozdìlení komponent nástroje do vrstev, èím¾ usnadòuje dal¹í vývoj a roz¹íøení nástroje. Pro efektivnìj¹í pou¾ití nástroje byla pøedstavena distribuovaná architektura nástroje, která pøedstavuje mo¾nost pou¾ít více fyzických strojù souèasnì pro testování cílové webové slu¾by. 

Implementace nástroje se nejdøíve soustøedila na vývoj testovací jednotky. Jakmile testovací jednotka splòovala základní po¾adavky, bylo pøistoupeno k integraci s proxy jednotkou do modulárního nástroje. Pro splnìní základních po¾adavkù na testovací nástroj, byla implementace zamìøena na snadnìj¹í ovládání ve formì grafického u¾ivatelského prostøedí. Po celou dobu implementace byl respektováno rozvr¾ení do vrstev, aby ve finální fázi mohla být zaèlenìna implementaci distribuované architektury. 	

Poslední èást diplomové práce se vìnuje sledování chování webových slu¾eb bìhem testování vytvoøeným nástrojem. Nejdøíve je popsáno jak nástroj nastavit, aby bylo mo¾né pou¾ít distribuovanou architekturu. Následnì je popsán zpùsob testování a vyhodnocení dat získaných nástrojem. V závìru kapitoly jsou shrnuty dosa¾ené výsledky pøi pou¾ití nástroje.

Tato práce splnila hlavní cíl a dokázala popsat chování vybraných slu¾eb pøi vysoké zátì¾i. Dále pak navázala na pøedchozí práci o injekci chyb do webových slu¾eb a roz¹íøila její výsledky. Díky distribuované architektuøe má nástroj ¹ir¹í paletu pou¾ití a je mo¾né jej integrovat s dal¹ími nástroji pro testování. Autor práce si vyzkou¹el práci ¹irokou ¹kálou technologii. Zejména zku¹enosti s návrhem a implementací rùzných postupù. Autor si cení zejména zku¹enosti s paralelním zpracováním a distribuovanou architekturou na platformì Java. Vytvoøený nástroj je prakticky pou¾itelný a je mo¾né jej pou¾ít pøi testování webových slu¾eb. 




\section{Mo¾nosti dal¹ího vývoje}
Nástroj má ¹iroký potenciál roz¹íøení a lze vylep¹ovat jádro testovací jednotky, ovládání GUI nebo dal¹í mo¾nosti HTTP dotazù. Zde budou uvedeny jen nìkterá roz¹íøení, která zvy¹ují jednoduchost a efektivnost nástroje.  

 Bìhem distribuovaného testu bylo zji¹tìno, ¾e je vhodné mít více proxy jednotek na nìkterém ze vzdálených strojù. Je mo¾né, ¾e slu¾ba mù¾e vyu¾ívat dal¹í slu¾by a bylo by tak mo¾né vkládat chyby do komunikace mezi jednotlivými slu¾bami. Pro spu¹tìní více proxy jednotek na vzdáleném stroji je potøeba implementovat dynamickou registraci instancí proxy jednotky na RMI serveru. Jako ideální se zdá být implementace návrhového vzoru továrna s vyu¾itím RMI.

Dále by bylo vhodné implementovat více strategii pro zátì¾ové testování, jako napøíklad odesílání HTTP dotazù po dávkách apod. Jeliko¾ je pou¾ita knihovna pro zpracování WSDL dokumentù, je mo¾né implementovat automatické vytvoøení kompletní testovací sady s pøedem nastavenými hodnotami. Roz¹íøeny mohou být dal¹í mo¾nosti nastavení HTTP spojení jako je pou¾ití HTTP autentizace nebo nastavení  SSL certifikátù.

Jako vhodné roz¹íøení se jeví vizualizace výstupù jednotlivých testovacích sad, aby bylo zøetelné chování slu¾eb pøi aktuální zátì¾i.
