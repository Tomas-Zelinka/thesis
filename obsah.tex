...
Proxy monitorovací jednotka be¾ící v nekoneèném cyklu pøijme zprávu (1) a vytvoøí nové vlákno pro jeho zpracování (2). Vlákno zpracuje zprávu a ode¹le jí kontroléru. Kontrolér pøedá zprávu injektoru(4,5) poruch, kde se provede analýza zprávy a její pøípadné naru¹ení. Kontrolér zastává funkci observer podle daného návrhového vzoru. Kontrolér tedy odesílá(6) zprávu do panelu GUI, kde je zobrazena. Zobrazovací panel GUI zde plní roli posluchaèe, který je dynamicky pøiøazen ke kontroléru. Po zobrazení je zpráva vlo¾ena na výstup a pokraèuje urèeným smìrem. Tento stav je nevyhovující, proto¾e pøi tomto stavu je komplikované vytvoøit více monitorovacích jednotek na více strojích.
...
\noindent Zmìna spoèívá v pøesunutí úlohy observer na monitorovací jednotku a kontrolér bude pouze delegovat registraci posluchaèù (monitorovacích panelù GUI) v monitorovací jednotkách. Pak je mo¾né mít více nezávislých monitorovacích jednotek na rùzných strojích. Tyto jednotky pak budou pouze odesílat získaná data do centrálního GUI, kde dojde k jejich zobrazení a zpracování. Kontrolér zde bude plnit funkci delegáta zpráv mezi monitorovacími jednotkami a zobrazovacími panely GUI.
...
Na obrázku \ref{fig:modelImpl} je model rozdìlení jednotlivých souèástí aplikace do navr¾ených vrstev. Do aplikaèní vrstvy jsou jsou umístìna u¾ivatelská rozhraní, testovací a proxy jednotky. Do transportní vrstvy je umístìn kontrolér, proto¾e se stará o transport dat a pøíkazù mezi sousedními vrstvami. Jak je z obrázku patrné u¾ivatelská rozhraní s grafickou nadstavbou jsou zodpovìdné za vytváøení dat, zatímco u¾ivatelské rozhraní z pøíkazové øádky má mo¾nost pouze odesílat pøíkazy a zprostøedkovat výstupy z aplikaèních jednotek. U¾ivatelská rozhraní komunikují pouze s kontrolérem a jsou odstínìna od odstaních operací.

Dále do aplikaèní vrstvy jsou umístìny aplikaèní jednotky, které jsou slou¾í  ke  vzdálené komunikaci s webovými slu¾bami a získávání dat. Aplikaèní jednotky v rámci aplikace komunikují pouze s kontrolérem a jsou tak odstínìny od u¾ivatelských rozhraní, ètení a ukládání dat.

Kontrolér je zodpovìdný za pøijímání dat od aplikaèní vrstvy a dle druhu komunikace deleguje data do aplikaèních jednotek nebo data odesílá datové vrstvì k ulo¾ení. Pokud vy¾aduje ètení dat, kontrolér deleguje zprávu do datové vrstvy a pøijme ji¾ data v po¾adovaném formátu. Hlavní úlohou kontroléru je delegace dat a pøíkazù mezi sousedními vrstvami.
...
Poslední vrstva reprezentuje ulo¾ená data na disku. Tyto data budou mít hierarchickou strukturu kvùli efektivnìj¹í orientaci.
\begin{figure}[h]
\begin{center}
...
Pro organizaci dat bylo zavedeno hierarchické schéma, které je patrné na obrázku. Na nejvy¹¹ím úrovni je koøenový adresáø, který není v grafickém u¾ivatelském rozhraní (dále jen GUI) patrný. Koøenový adresáø se nachází v pracovním adresáøi aplikace a pokud není pøi startu aplikace pøítomen , je vytvoøen.
...
Centrální souèást aplikace, která má za úkol delegovat pøíkazy z u¾ivatelských rozhraní do aplikaèních jednotek a zprostøedkovat pøenos dat mezi jednotlivými souèástmi. Dal¹í dùle¾itá úloha spu¹tìní vláken pro aplikaèní jednotky. 
...
\subsection*{Testovací jednotka}
Testovací jednotka je jádrem této práce. Jejím hlavním úkolem je získat odpovìï od webové slu¾by. Základem jsou data pro testování. Data jsou ji¾ ve formì instance tøíd, která obsahují potøebné informace. Testovací jednotce jsou pøedány instance tøíd pro nastavení testu a HTTP po¾adavku. Z dat nastavení testu je pou¾it poèet vláken, který se má pou¾ít pro jeden testovací pøípad. Testovací jednotka podle pøidìleného poètu dopøedu vytvoøí instance vláken pomocí tøídy   {\it Executors}. Tøída  {\it Executors} doká¾e dopøedu vytvoøit fixní poèet vláken a spustit je a¾ jsou potøeba. Dopøedu vytvoøená vlákna ¹etøí výpoèetní prostøedky, proto¾e pøi jejich spu¹tìní odpadá re¾ie na jejich vytvoøení. Jednotlivá vlákna implementují rozhraní  {\it Callable}, tak¾e pøed ukonèením vlákna je vrácena hodnota. Pøi spu¹tìní je  vlákna je návratová hodnota pøedána do instance tøídy {\it Future}. Instance  tøídy {\it Future} zaji¹»uje asynchronní zpracování výsledku vrácené návratové hodnoty vlákna.

Vlákna jsou spou¹tìny v poèítaném cyklu a jsou pøidìlena do pole obsahující instance tøíd {\it Future}. Po spu¹tìní následuje dal¹í cyklus, který blokujícím zpùsobem èeká na dokonèení jednotlivých vláken. Pøi ka¾dém pøijetí výsledku od vlákna je tento výsledek testovací jednotkou odeslán do zobrazujícího panelu, kde je výsledek okam¾itì zobrazen. Po skonèení v¹ech vláken jsou v¹echna vlákna ukonèena øádným zpùsobem pomocí instance tøídy  {\it Executors}.
...
\\\\
\noindent Hlavní panely jsou ètyøi:
\begin{itemize}
\item {\bf Panel editoru}\\
ffdsf
\item {\bf Testovací panel}\\
ffdsf
\item {\bf Panel proxy monitoru}\\
ffdsf\\
\item {\bf Panel pro statistiky}\\
ffdsf
\end{itemize}
\subsection*{Rozhraní pøíkazové øádky}
Rozhraní pøíkazové øádky je fakticky hlavním rozhraním celého nástroje. Nástroj je mo¾né spustit pouze z pøíkazové øádky. Av¹ak je mo¾né pou¾ít skriptovací nástroje jako je Bash v OS Linux nebo dávku .bat v pøípadì OS Windows. Nástroj disponuje tìmito pøíkazy:

\begin{itemize}
\item {\bf Spu¹tìní grafického u¾ivatelského rozhraní }\\
asdfd
\item {\bf Zobrazení nápovìdy}\\
dsfsdf
\item {\bf Spu¹tìní testu}\\
asdfsdf
\item {\bf Zastavení proxy jednotky}\\
asdfsdf
\\
\end{itemize}
...
V této èásti bude popsáno jaké funkce obsahuje editor dat a jaké prostøedky vedly jeho vytvoøení. V èásti schéma testovacích dat je popsáno k èemu slou¾í jednotlivé polo¾ky rozlo¾ení dat a jakým zpùsobem je mo¾né je vyu¾ít. V dal¹í èásti jsou popsány jednotlivé souèásti editoru, které je nutné pou¾ít k vytvoøení po¾adovaných dat k testování. 
...
V této èásti je popsáno jaké funkce je mo¾no vyu¾ít k ovládání testovací jednotky a jakým zpùsobem se testovací jednotka ovládá.

\subsection*{Ovládání testovací jednotky}
asdfasdf
\subsection*{Pøíjem a zobrazení dat od webové slu¾by}
asdfasdf

\section{Vyhodnocení testu}
asdfasdf
\newpage
