...
Návrh obsahuje distribuovanou architekturu a je vhodné uvést model vrstev aplikace. Rozdìlit aplikaci pøi návrhu do vrstev je výhodné, proto¾e umo¾òuje dekomponovat slo¾itìj¹í problémy na podproblémy a sní¾it tak slo¾itost následné implementace. Obrázek ukazuje rozlo¾ení jednotlivých vrstev.  
...
%\begin{figure}[h]
%\begin{center}
% \includegraphics[scale=0.8]{pic/vrstvy.png}
%\caption{Model distribuované architektury. }
%\label{fig:dist}
%\end{center}
%\end{figure}
...
Tato kapitola se vìnuje popisu implementace návrhu, který byl pøedstaven v pøede¹lé kapitole.  Vytvoøení jednotlivých èástí programu je postupnì popsáno a u ka¾dé èásti jsou uvedeny pou¾ité technologie a postupy, které byly pou¾ity. Kapitola je rozdìlena na sekce tak, jak postupnì implementace aplikace splòuje zadání. Postupnì jsou pøedstavovány èásti od tìch základních jako volba implementaèního prostøedí, zpùsob vytvoøení testovacích dat a spu¹tìní základního testu. Dále pak je popsáno øe¹ení jednotlivých bodù ze zadání, èili integrace s pøedchozím øe¹ením, vylep¹ení injekce chyb a mo¾nost integrace s testovacími nástroji. Implementace distribuované architektury je popsána jako poslední, proto¾e se jedná o vylep¹ení vùèi základním po¾adavkùm na aplikaci.
...
Jako základ pro testování webových slu¾eb je nejdøíve nutné vytvoøit testovací data, která budou odesílána na vstup webové slu¾by.
