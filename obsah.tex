...
\noindent Mo¾nosti pro vylep¹ení chování monitorovací jednotky:
\begin{itemize} 
...
Pøede¹lá verze podporovala zpracování pouze formátu XML. V ostatních pøípadech nebyla data proxy jednotkou vùbec zaznamenána. Nyní je proxy jednotka bude nejprve èíst obsah parametru {\it Content-Type} a podle toho rozhodne, jakým zpùsobem data zpracuje. Pokud parametr nebude pøítomen budou data odeslána na výstup bez zpracování.
\noindent Dal¹í mo¾nosti pro podporu dal¹ích datových formátù injektoru poruch:
\begin{itemize} 
\item{ \bf JSON}\\
JSON je roz¹íøený datový formát pou¾ívaný pøedev¹ím webovými slu¾bami REST charakteru. Data reprezentována JSON formátem jsou v textové podobì. Lze tedy uplatnit stávájící injekce poruch. Pokud v¹ak je nutné komplexnìj¹í injekci, jako napøíklad zmìna urèité promìnné, je nutné nejdøíve zpracovat data vhodným zpùsobem. Pro komplexnìj¹í zpracování je mo¾no vyu¾ít volnì dostupných knihoven.\\
\item{ \bf YAML}\\
YAML je svou charakteristikou i pou¾itím velmi podobný formátu JSON. Také se jedná o reprezentaci dat v textovém formátu. Bohu¾el implementace nebyla nalezena knihovna, která by umo¾òovala zpracování dle modelu YAML bez pøetypování na známý Java objekt nebo kolekci. Pokud není známa implementace objektu, není mo¾né jednoznaènì urèit typ objektu, na který má být pøeètená data pøetypována. Dostupné knihovny pro JSON formát umo¾òují manipulaci s JSON pomocí modelu, tedy není nutné pøetypování na známý objekt nebo kolekci. Tento formát tedy nebude proxy jednotkou pøímo podporován. Z vý¹e uvedených dùvodù bude s tímto formátem zacházeno jak s neformátovaným textem.  \\
\end{itemize}
