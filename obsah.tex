...
Pro zpracování obsahu ve formátu JSON je pou¾ita knihovna  {\it GSON}. Podpora formátu v rámci vytvoøeného nástroje znamená, ¾e textové výstupy tohoto formátu jsou pøehlednì naformátovány a je mo¾né pou¾ít obsah zpráv ve formátu JSON pøi injekci poruch.
...
Tato kapitola se zabývá pou¾itím vytvoøeného nástroje. V pøedchozí práci .. bylo pøedlo¾eno schéma {\it wsCrash} pro klasifikaci poruch, které vychází z této práce .. . Tato práce se této klasifikace bude dr¾et a pøi testování se bude sna¾it vzniklé chyby zaøadit do této klasifikace. 
\begin{itemize}
...
\end{itemize}
Zátì¾ové testování bude nejprve zkoumat, jestli slu¾ba správnì zpracovává více po¾adavkù v krátkém èasovém okam¾iku. Tímto zpùsobem mù¾eme ovìøit základní pøedpoklad správného zpracování více po¾adavkù. Následnì mù¾eme spustit zátì¾ový test a hledat hranici pou¾itelnosti na základì doby odezvy testované slu¾by. Hranici mù¾eme nastavit èasovým limitem HTTP spojení a sledovat od jakého poètu odeslaných po¾adavkù a spu¹tìných vláken zaène klientská aplikace uzavírat spojení.
\newpage
...
Pro ovìøení funkce nástroje byly vytvoøeny ukázkové webové slu¾by. Jako aplikaèní server byl pou¾it JBoss AS verze 7.0.2, který je spu¹tìn na jiném fyzickém stroji ne¾ testovací nástroj. Slu¾ba {\it BookStore} je pou¾ívá SOAP protokol a slu¾ba {\it Users} je vytvoøena na REST principech. Slu¾ba {\it BookStore} komunikuje zprávami v XML formátu zatímco slu¾ba {\it Users} komunikuje zprávami ve formátu JSON.


\section{Testování webových slu¾eb s  dostupným zdrojovým kódem}
Tato sekce se zabývá testováním slu¾eb se známým kódem. Vytvoøené slu¾by jsou pouze ukázkové aplikace, do které jsou zámìrnì implementovány jako chybné. V prvním pøípadì se jedná o webovou slu¾bu, která pøidává a odebírá knihy ze skladu. Zámìrnì byla opomenuta synchronizace vláken, které pøistupují k poèítadlu knih.
\subsection*{Zátì¾ový test SOAP webové slu¾by {\it BookStore}}
\begin{tabular}{| c | c | c | c |}
\hline
vlákno & sekvence & výsledek& doba odezvy [ms]\\ \hline
0&0 & 2& 31\\ \hline
...
\subsection*{Zátì¾ový test REST webové slu¾by {\it Users}}
asdsaf
...
Prùbìh testování ukázal, ¾e pøi nevhodné implementaci je mo¾né se setkat s chybným chováním slu¾by ji¾ pøi nízkém poètu vláken. Dále bylo zji¹tìno, ¾e pøi vy¹¹ím zatí¾ení je mo¾no pozorovat nìkolik negativních rysù chování slu¾by. V nejlep¹ím pøípadì je vrácen chybový kód 500 a patøièná zpráva. Dále bylo zji¹tìno, ¾e pøi vysokém zatí¾ení je mo¾né se setkat s dvìma druhy èasových limitù. Ov¹em musíme pøedpokládat, ¾e jsou nastaveny. V opaèném pøípadì mù¾e celá aplikace zamrznout na neurèitou dobu. Pokud je slu¾ba opravdu pøetí¾ena není s ní mo¾no ani navázat spojení a je uplatnìn èasový limit schránky (socket). Pokud zatí¾ení není tak kritické, je spojení navázáno, ale je mo¾né se setkat s dobou èekání na data v øádech sekund. Zde se uplatní èasový limit na HTTP po¾adavek.
...
Poslední èást diplomové práce se vìnuje sledování chování webových slu¾eb bìhem testování vytvoøeným nástrojem. Nejdøíve je popsáno jak nástroj nastavit, aby bylo mo¾né pou¾ít distribuovanou architekturu. Následnì jsou popsány sledované parametry a zpùsob vyhodnocení dat, získaných nástrojem. V poslední èásti kapitoly jsou shrnuty dosa¾ené výsledky pøi pou¾ití nástroje.

Tato práce splnila hlavní cíl a dokázala popsat chování vybraných slu¾eb pøi vysoké zátì¾i. Dále práce ukázala, jak lze vyu¾ít spojení zátì¾ového testování s injekcí chyb. Díky distribuované architektuøe jsou má nástroj ¹ir¹í paletu pou¾ití a je mo¾né jej integrovat s dal¹ími nástroji pro testování. Autor práce si vyzkou¹el práci ¹irokou ¹kálou technologii. Zejména zku¹enosti návrhem a implementací rùzných postupù. Autor si cení zejména zku¹enosti s paralelním zpracováním a distribuovanou architekturou na platformì Java. Vytvoøený nástroj je prakticky pou¾itelný a je mo¾né jej pou¾ít pøi sestavení robustnostní charakteristiky testované slu¾by. 
...
Nástroj má ¹iroký potenciál roz¹íøení a lze vylep¹ovat jádro testovací jednotky, ovládání GUI nebo dal¹í mo¾nosti HTTP dotazù. Zde budou uvedeny jen nìkterá roz¹íøení, která zvy¹ují jednoduchost a efektivnost nástroje.  
 ...
 Bìhem distribuovaného testu bylo zji¹tìno, ¾e je vhodné mít více proxy jednotek na nìkterém ze vzdálených strojù. Je mo¾né, ¾e slu¾ba mù¾e vyu¾ívat dal¹í slu¾by a bylo by tak mo¾né vkládat chyby to komunikace mezi jednotlivými slu¾bami. Pro spu¹tìní více proxy jednotek na vzdáleném stroji je potøeba implementovat dynamickou registraci instancí proxy jednotky na RMI serveru. Jako ideální se zdá být implementace návrhového vzoru továrna s vyu¾itím RMI.
